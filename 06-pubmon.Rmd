...
## 2026-2027 Postdoc Yr2
[Unpaired data as a first-order challenge in single-cell and spatial proteomics](https://www.nature.com/articles/s41587-026-03074-8)

Single-cell and spatial proteomics are rapidly maturing, with mass spectrometry and imaging-based technologies now capable of quantifying tens to thousands of proteins across cells and tissues. Yet, as measurement resolution, depth and throughput improve, progress is increasingly constrained not by experimental sensitivity but by the ability to integrate proteomic data across experiments, modalities and spatial contexts. In practice, most proteomic measurements are unpaired to transcriptomic or epigenomic references: proteins are measured in different cells, adjacent tissue sections or separate experiments. These partial, misaligned, and heterogeneous observations represent the dominant data regime in current studies, but they remain poorly supported by existing analytical frameworks1.

Much of the single-cell analysis ecosystem was developed under assumptions that do not hold for proteomics. Transcriptomics workflows typically rely on dense, paired measurements, with one-to-one correspondences between cells across conditions or modalities. In contrast, proteomics datasets, particularly those generated by mass spectrometry or spatial imaging, are often sparse, incomplete, and non-overlapping by design. Spatial proteomics adds further complexity: protein maps may need to be registered across tissue sections, platforms or resolutions, frequently without a shared cellular index. Treating these scenarios as exceptions rather than defaults leads to brittle analyses, limits cross-study comparison, and risks obscuring biological signal behind technical mismatches2,3. Taken together, these constraints indicate that pairing should no longer be treated as the default assumption for multimodal proteomics analyses.

We argue that **unpaired integration** should be treated as a first-order design constraint for future single-cell and spatial proteomics methods. This prioritization should precede further method development optimized for fully paired or near-ideal multimodal datasets, which remain the exception rather than the rule in proteomics. This shift has concrete implications. Algorithmically, it prioritizes approaches that align distributions rather than cells, that learn from partial observations, and that explicitly model uncertainty arising from missing modalities. Conceptually, it reframes integration as a mapping problem, linking proteomic measurements to reference atlases, spatial coordinates or multimodal embedding, rather than a requirement for matched profiles. In practice, it demands benchmarks, evaluation metrics and metadata standards that reflect realistic experimental designs rather than idealized paired datasets. Emerging community efforts toward interoperable data structures, reproducible workflows and best-practice guidelines within ecosystems such as the scverse project4 and related multimodal initiatives represent early steps in this direction. However, systematic benchmarking frameworks tailored specifically to unpaired and partially observed proteomics data remain largely underdeveloped.

Shared computational infrastructure is necessary to support this shift, but it should be viewed as an enabling layer rather than an endpoint. Community frameworks such as scverse4 illustrate how interoperable data containers and modality-agnostic representations can lower barriers to integration by standardizing how heterogeneous proteomic outputs are stored, accessed and combined. By supporting multimodal data structures and spatial representations within a common analytical ecosystem, these infrastructures enable the systematic development and comparison of unpaired integration methods. However, infrastructure alone cannot solve the problem: without methods and standards that explicitly embrace partial and misaligned data, interoperability risks formalizing existing limitations5,6.

Addressing unpaired integration now is timely and essential. As spatial and single-cell proteomics scale and multimodal atlases expand, the gap between what can be measured and what can be meaningfully integrated will only widen. Recent advances underscore both the promise of spatial proteomics and the urgency of robust integration strategies7. Prioritizing unpaired data as a core analytical challenge will enable more robust cross-study synthesis, fairer method benchmarking and more faithful biological interpretation. Doing so requires coordinated effort across experimental design, data standards and computational methods, but it is essential if proteomics is to become a truly integrative component of single-cell and spatial biology8. Absent such a shift, proteomics risks remaining analytically peripheral to **multimodal single-cell and spatial biology**, despite rapid experimental advances.
